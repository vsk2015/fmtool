...
class Flow(object):
    ...
    def __init__(self, node, node_of_name, cookie=None, table=None, name=None):
        ...
        self.of_config_id = None
        self.of_config_version = None
        self.of_operational_id = None
        self.of_operational_version = None
        self.switch_id = None
        self.switch_version = None
        self.cookie = cookie
        self.table = table
        self.name = name
    ...
    def check(self):
        ...
        if len(self.of_operational) > 1:
            logging.error("flow %s duplicated in operational. %s",
                          self.flowid, self._get_info_msg())
        if len(self.switch) > 1:
            logging.error("flow %s duplicated in switch. %s",
                          self.flowid, self._get_info_msg())
        if config and not switch:
            logging.error("flow %s is not running in the switch. %s",
                          self.flowid, self._get_info_msg())
        if config and not operational:
            logging.error("flow %s not found in operational datastore. %s",
                          self.flowid, self._get_info_msg())
        if config and not self.calculated and str(self.of_config[0]['id']) not in CALCULATED_EXCEPTIONS:
            logging.error("flow %s not found in calculated flows. %s",
                          self.flowid, self._get_info_msg())
        if not config and switch:
            logging.error("flow %s running in switch but not configured. %s",
                          self.flowid, self._get_info_msg())
        if not config and operational:
            logging.error("flow %s found operational datastore but not in configuration. %s",
                          self.flowid, self._get_info_msg())
        if config and switch and self.of_config_version != self.switch_version:
            logging.error("flow %s config and switch version is different. %s",
                          self.flowid, self._get_info_msg())
        if config and operational and self.of_config_version != self.of_operational_version:
            logging.error("flow %s config and operational version is different. %s",
                          self.flowid, self._get_info_msg())
        if config and switch and self.of_config_id != self.switch_id:
            logging.error("flow %s config and switch id is different. %s",
                          self.flowid, self._get_info_msg())
        if config and operational and self.of_config_id != self.of_operational_id:
