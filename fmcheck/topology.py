...
from fmcheck.switch import get_switch_type
...
class Topology(object):
        ...
        try:
            ...
            for switch in self.switches.values():
                ...
                for group in switch.groups.values():
                    if not group.check():
                        group_errors.append(group.groupid)
                for flow in switch.flows.values():
                    if not flow.check():
                        flow_errors.append(flow.cookie)
            ...
            if len(group_errors) > 1:
                logging.error("Errors with Groups")
                result = False
            if len(flow_errors) > 1:
                logging.error("Errors with Flows")
                result = False
            if result:
    ...
    def load_openflow_elements(self,dbname,fmcheckrun):
        ...
        if nodes is not None:
            for node in nodes:
                nodeid = node['node-id']
                sr = node.get(ctrl.get_container_fm('sr:sr'))
                if sr is not None:
                    self.process_calculated(sr)
