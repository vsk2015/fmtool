...
class Controller(object):
    ...
    def __init__(self, props, controller_vip):
        ...
        self.timeout = 30 if not props.get(
    ...
    def is_lumina(self):
        return self.get_fm_prefix() == LUMINA_FLOW_MANAGER_PREFIX

    def is_brocade(self):
        return self.get_fm_prefix() == BROCADE_FLOW_MANAGER_PREFIX
    ...
    def get_fm_prefix(self):
        if not self.fm_prefix:
            ...
            REQUEST_URL = '/' + LUMINA_FLOW_MANAGER_PREFIX + REST_URL_PATH
    ...
    def get_fm_eline_stats_url(self):
        return self.get_operations_fm_url(REST_URL_ELINE_STATS)

    def get_fm_etree_stats_url(self):
        return self.get_operations_fm_url(REST_URL_ETREE_STATS)
    ...
    def get_eline_stats(self, filters=None):
        logging.debug(self.get_operational_fm_url(REST_URL_ELINE))
        resp = self.http_get(self.get_operational_fm_url(REST_URL_ELINE))
        if resp is None or resp.status_code != 200 or resp.content is None:
            logging.error(
        ...
        for eline in data['elines']['eline']:
            if contains_filters(filters, eline['name']):
                ...
                resp = self.http_post(self.get_fm_eline_stats_url(), '{"input":{"name": "' + eline['name'] + '"}}')
    ...
    def get_eline_summary(self, filters=None):
        ...
        for eline in data['elines']['eline']:
            if contains_filters(filters, eline['name']):
                ...
                resp = self.http_get(self.get_config_fm_url(REST_URL_PATH) + '/path/{}'.format(eline['path-name']))
                if resp is None or resp.status_code != 200 or resp.content is None:
    ...
    def get_etree_stats(self, filters=None):
        logging.debug(self.get_operational_fm_url(REST_URL_ETREE))
        resp = self.http_get(self.get_operational_fm_url(REST_URL_ETREE))
        if resp is None or resp.status_code != 200 or resp.content is None:
            logging.error(
                'no data found while trying to get openflow information %s', resp.status_code)
            return
        ...
        for etree in data['etrees']['etree']:
            if contains_filters(filters, etree['name']):
                ...
                resp = self.http_post(self.get_fm_etree_stats_url(), '{"input":{"name": "' + etree['name'] + '"}}')
                print json.dumps(json.loads(resp.content), indent=2)

    def get_etree_summary(self, filters=None):
        logging.debug(self.get_fm_etree_stats_url())
        resp = self.http_get(self.get_fm_etree_stats_url())
        if resp is None or resp.status_code != 200 or resp.content is None:
            logging.error(
                'no data found while trying to get openflow information %s', resp.status_code)
            return
        ...
        for etree in data['etrees']['etree']:
            if contains_filters(filters, etree['name']):
                ...
                resp = self.http_get(self.get_config_fm_url(REST_URL_TREEPATH)+ '/treepath/{}'.format(etree['treepath-name']))
                if resp is None or resp.status_code != 200 or resp.content is None:
    ...
    def get_sr_summary(self, switches, source, destination, srnodes=None):
        if srnodes is None:
            srnodes = self._get_sr_nodes_paths(switches)
    ...
    def _get_sr_nodes_paths(self, switches):
        ...
        for node in nodes:
            ...
            if not nodeid in switches:
                continue
            if self.is_lumina():
                sr = node.get('lumina-flowmanager-sr:sr')
            else:
                sr = node.get('brocade-bsc-sr:sr')
            if sr is None:
                continue
            srnodes[nodeid] = {
                'mpls-label': sr.get('mpls-label'), 'primary-paths': {}}

            cppaths = sr.get('calculated-primary-paths')
            if cppaths is None:
                continue
            ...
            if connectors is not None:
                for connector in connectors:
                    ...
                    port_number = port_number if port_number else connector.get(
                        'port-number')
                    port_number = port_number if port_number else "unknown"
