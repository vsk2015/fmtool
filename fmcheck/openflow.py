...
# This cache is almost completely useless as it does not appear to be hit
cache = {}


def get_from_cache_object(ctrl, url):
    if ctrl.name in cache and url in cache[ctrl.name]:
        logging.debug("Getting from cache[{}][{}]".format(ctrl.name, url))
        return cache[ctrl.name].get(url)


def add_to_cache_object(ctrl, url, data):
    logging.debug("Adding to cache cahce[{}][{}]".format(ctrl.name, url))
    if ctrl.name in cache and url not in cache[ctrl.name]:
        cache[ctrl.name][url] = data
    else:
        cache[ctrl.name] = {url: data}
...
def get_from_api(ctrl, url, use_cache=True):
    ...
    if not use_cache or not data:
        ...
        add_to_cache_object(ctrl, url, data)
